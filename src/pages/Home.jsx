...
export default function Home() {
  ...
  const { hero, stats, features, programs, ctaImage } = content;
  const headlineLines = (hero.title || "").split("\n").filter(Boolean);

  return (
    <div>
      {/* HERO — full-bleed parallax photo, split-text headline, cursor glow */}
      <ParallaxBanner src={hero.image} alt={hero.title} height="min-h-[92vh]" strength={90}>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8 h-full flex flex-col justify-end pb-16 pt-40 md:pt-52">
          ...
          <h1 className="font-display leading-[0.9] uppercase text-[13vw] sm:text-6xl md:text-8xl drop-shadow-[0_4px_24px_rgba(0,0,0,0.6)]">
          ...
          >
            <MagneticButton>
              <NavLink
                to="/contact"
                className="block bg-accent text-bg font-mono text-xs uppercase tracking-widest font-bold px-7 py-4 hover:bg-ink transition-colors"
              >
                Book a free session
              </NavLink>
            </MagneticButton>
      ...
      <Marquee text="NO EXCUSES \u2014 SHOW UP \u2014 TRACK PROGRESS \u2014 REPEAT" />
      ...
      <Marquee text="STRENGTH \u2014 CONDITIONING \u2014 MOBILITY \u2014 RECOVERY" reverse />
