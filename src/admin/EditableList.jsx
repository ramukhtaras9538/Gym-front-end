...
export default function EditableList({ page, section, title, items = [], fields, onChange }) {
  ...
  function startEdit(item) {
    const f = {};
    fields.forEach((field) => (f[field.name] = item[field.name] ?? (field.type === "number" ? 0 : "")));
    setForm(f);
  ...
  function handleFieldChange(name, type, value) {
  ...
  async function handleSave(e) {
    ...
    setError("");
    try {
      // Upload any files in form fields first
      const payload = { ...form };
          ...
          {items.map((item) => (
            <li key={item._id} className="py-4 flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                {fields[1] && (
                  <p className="text-muted text-sm mt-1 line-clamp-2">{item[fields[1].name]}</p>
                )}
              </div>
